...
let rec hasUnderscore = function
  | []       -> false
  | ('_'::_) -> true
...
let render_atom env atom =
  ...
  in
  let s =
    (* TODO: Add translation rules, and normal naming instead of latex *)
    let open Il.Atom in
    match atom.it with
...
let rec label_line id1 = function 
  | []               -> ""
  | [Elem x]         -> x
  | (Elem x)::Nl::xs -> id1 ^ x ^ "\n" ^ label_line id1 xs
  | (Elem x)::xs     -> id1 ^ x ^ "\n" ^ label_line id1 xs
...
and render_typcase2 env (_atom, (t, prems), _hints) = 
  render_typ2 env t ^ 
  if prems = [] then "" else render_conditions env "&&&&" prems

and render_typ2 env t id1 =
  match t.it with
  | StrT tfs -> 
    concat_map_nl ",\\; " "\\\\\n  " (render_typfield env) tfs
  ...
  (* | CaseT (dots1, ts, tcases, dots2) -> *)
  | CaseT (_, _, tcases, _) -> 
    label_line id1 (map_nl_list (render_typcase2 env) tcases)
  
(* let map_nl_list f xs = List.map (function Nl -> Nl | Elem x -> Elem (f x)) xs *)


  (* | ConT tcon -> render_typcon env tcon *)
  | ConT _    -> "ssssssssssssssss"
  (* | RangeT tes -> altern_map_nl " ~|~ " "\\\\ &&|&\n" (render_typenum env) tes *)
  | RangeT _  ->"dddddddddddddddd"
  | _         -> "xxxx" (* render_exp env (exp_of_typ t) *)

let render_typdef env d =
  match d.it with
  | TypD (id1, _id2, args, t, _) ->
    let id1' = render_typdeco env id1 (* Do translation rules *) in
    
    render_typdeco env id1 ^ render_apply render_typid render_exp env env.show_typ id1 args ^ 
    " ::= " ^ 
    render_typ2 env t id1'
  | _ -> assert false

(* let rec merge_typdefs2 env defs id1 = 
  match defs.it with
  | [] -> ""
  | d::ds -> 
    d ^ ". " ^ id1 ^ " ::= " ^ render_typ2 env d ^ "\n" ^ merge_typdefs2 env ds id1
    
let render_typdef env d =
  match d.it with
  | TypD (id1, _id2, args, t, _) ->
    let id1' = id1 (* Do translation rules *) in
    merge_typdefs2 env t id1'
    (* id1' ^
...
let rec render_defs env = function
  ...
  | d::ds' as ds ->
    ...
    match d.it with
    | TypD _ ->
      let syntax_defs = merge_typdefs ds in
      render_sep_defs (render_typdef env) syntax_defs
